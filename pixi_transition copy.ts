    ...
    for (let i = initializer; i < limit; i++) {
      ...
      if (!animation.transition) {
        ...
        for (let index = 0; index < animation.images!.length; index++) {
          ...
          if (
            ...
            if (Number(cellImage.isVideo) === 1) {
              ...
              const videoSource: HTMLMediaElement = (videoTexture.baseTexture.resource as PIXI.resources.VideoResource)
        ...
        if (animation.slide && animation.animations?.length) {
          ...
          for (const slideAnimation of animation.animations) {
            ...
            if (slideAnimation.isTransitionIn || slideAnimation.isTransitionOut) {
              ...
              if (resource && !resource.renderable) {
                ...
                if (animation.slide && animation.animations?.length) {
                  const animIndex = animation.animations.findIndex((anim) => anim.objectId === slideAnimation.objectId);
          ...
          gapsToFillWithUserVideo.forEach((item) => {
            const timeline = gsap.timeline();
        ...
        if (animation.type) {
          ...
          switch (animation.type.toLowerCase()) {
            case 'fadeout':
              const promises = this.resources.map((obj) => {
              ...
              masterTimeline.add(timeline, (animation?.timestamp || 0) / 1000);
