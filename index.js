...
const renderer = new PIXI.autoDetectRenderer(200, 200, {
    antialias: true,
    transparent: true,
    ...
    if (vid) {
        ...
        const getFrames = () => new Promise(resolve => {
            if (HTMLVideoElement.prototype.requestVideoFrameCallback) {
                ...
                const drawingLoop = async (timestamp, frame) => {
                    ...
                    if (!video.ended) {
                        await video.requestVideoFrameCallback(drawingLoop);
                        console.log(videoFrames.length, ' frames')
                    } else {
                        console.log('gotten all frames - ', videoFrames.length)
                        stopped = true;
                    }
                };
        ...
        videoPromise = () => new Promise(async (resolve) => {
            if (!videoFrames.length) {
                console.log('waiting to get frames');
                await getFrames()
                console.log('waiting to get frames - DONE');

            }
            const base = await new PIXI.BaseTexture(videoFrames[index])
